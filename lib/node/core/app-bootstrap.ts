...
import type { MZRouter } from '../http';
import type { MZInterceptor } from '../http/interceptor';
...
export function appBootstrap(
  ...
  (AppEnvironment.instance as WithWritable<AppEnvironment>).bootstrapOptions =
    options;
  ...
  if (options.useParseCookie) {
    app.use(
      cookieParser(options.parseCookieSecret, options.parseCookieOptions),
    );
  }
  ...
  if (options.appAutoListen) {
    app.listen(
      AppEnvironment.instance.APP_PORT,
      AppEnvironment.instance.APP_HOST,
      function () {
