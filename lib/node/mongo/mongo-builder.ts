import { isArray, isObject, toArray, toString } from 'lodash';
...
> = 0 extends A
  ...
  : string extends A
  ? (
      val: A | RegExp | RequireOnlyOne<mongoose.QuerySelector<A>>,
    ) => IMongoBuilder<T>
...
export function throwOnMongoBuilderQueryResultError(
  result: MongoBuilderQueryResult<any>,
) {
  if (result.error) {
    throw result.error;
  }
...
export class MongoBuilder<T, ResultDoc = mongoose.HydratedDocument<T>> {
  ...
  public filterIds(
    ...
    ids = toArray(ids!).filter((item: string) => toString(item) !== '');
    ...
    return new Promise((resolve) => {
      try {
        ...
        this.model
          ...
          .on('error', (err) =>
            resolve(new MongoBuilderQueryResult<any>([], err)),
          );
      } catch (error: any) {
  ...
  async findAndCount(): Promise<
    MongoBuilderQueryResult<[Array<ResultDoc>, number]>
  > {
    try {
