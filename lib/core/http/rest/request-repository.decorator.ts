...
> extends HttpRestRequestRepositoryOptions<
    Response,
    Params,
    Body,
    Query,
...
interface IHttpRestRequestRepository {
  <
    ...
    File extends undefined | Constructor = undefined,
  >(
    options: HttpRestRequestRepositoryOptions<
      Response,
      Params,
      Body,
      Query,
...
HttpRestRequestRepository.get = (target: any) => {
  return (
    ...
    (httpRestRequestRepository.get(target) ||
      httpRestRequestRepository.get(target.constructor) ||
      httpRestRequestRepository.get(target.prototype) ||
      httpRestRequestRepository.get(target.__proto__))
  );
};
